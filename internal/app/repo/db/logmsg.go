...
func (r *LogMsgRepo) GetLevelCount() ([]entity.LogMsgLvlCount, error) {
	var results []entity.LogMsgLvlCount
	err := r.dbStorage.
		Model(&entity.LogMsg{}).
...
func (r *LogMsgRepo) GetWithLevel(level string) ([]entity.LogMsgWithLvl, error) {
	var results []entity.LogMsgWithLvl
	err := r.dbStorage.
		Model(&entity.LogMsg{}).
		Where("level = ?", level).
